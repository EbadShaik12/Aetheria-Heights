...
const profileSchema = new mongoose.Schema(
  {
    ...
    hotelName: { type: String, default: 'Aetheria Heights' },
    location: { type: String, default: '1 Ocean Drive, Azure Coast' },
...
const INITIAL_HOTELS = [
  {
    name: "Aetheria Heights",
    location: { lat: 19.0760, lng: 72.8777 }, // Mumbai
    pricePerNight: 8500,
    rating: 4.9,
    address: "1 Ocean Drive, Mumbai",
    available: true
  },
  {
    name: "Aetheria City Center",
    location: { lat: 19.0800, lng: 72.8800 },
    pricePerNight: 6200,
    rating: 4.5,
    address: "45 Business Park, Mumbai",
    available: true
  },
  {
    name: "Aetheria Coastal Retreat",
    location: { lat: 19.0900, lng: 72.8600 },
    pricePerNight: 9500,
    rating: 4.8,
    address: "12 Beachside Road, Mumbai",
    available: true
  },
  {
    name: "Aetheria Airport Inn",
    location: { lat: 19.0950, lng: 72.8750 },
    pricePerNight: 4500,
    rating: 4.2,
    address: "Near T2 Terminal, Mumbai",
    available: true
  }
];

// Initialize Mock Hotels if empty
const initializeHotels = async () => {
  try {
    const count = await Hotel.countDocuments();
    if (count === 0) {
      await Hotel.insertMany(INITIAL_HOTELS);
      console.log('✅ Initial mock hotels created');
    }
  } catch (err) {
    console.error('Failed to initialize hotels:', err);
  }
};
initializeHotels();
...
const sendNewsletterEmail = async (subscriberEmail) => {
  let hotelName = 'Our Luxury Resort';
  try {
    const adminProfile = await Profile.findOne({ role: 'ADMIN' });
    if (adminProfile && adminProfile.hotelName) {
      hotelName = adminProfile.hotelName;
    }
  } catch (error) {
    console.error("Could not fetch hotel name for newsletter:", error);
  }

  const mailOptions = {
    from: process.env.EMAIL_USER || 'your-email@gmail.com',
    to: subscriberEmail,
    subject: `Welcome to ${hotelName} - Discover Luxury Like Never Before!`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); color: white; padding: 20px;">
        <div style="text-align: center; padding: 40px 0;">
          <h1 style="color: #ffd700; font-size: 32px; margin: 0; font-family: serif;">✦ ${hotelName} ✦</h1>
        ...
        <div style="background: rgba(255, 255, 255, 0.05); padding: 30px; border-radius: 15px; margin: 20px 0;">
          ...
          <div style="background: rgba(255, 215, 0, 0.1); padding: 20px; border-radius: 10px; margin: 20px 0; border-left: 4px solid #ffd700;">
            <h3 style="color: #ffd700; margin: 0 0 15px 0;">Discover ${hotelName} - Your Complete Luxury Resort Experience:</h3>
        ...
        <div style="text-align: center; padding: 20px; border-top: 1px solid rgba(255, 255, 255, 0.1); margin-top: 30px;">
          <p style="color: #b8c5d6; font-size: 14px;">
            ${hotelName} | Luxury Resort & Digital Experience<br>
    ...
    const testMailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px;">
          <h2 style="color: #ffd700;">✦ ${hotelName} Email Test ✦</h2>
...
app.get('/api/hotel-config', async (req, res) => {
  try {
    const adminProfile = await Profile.findOne({ role: 'ADMIN' });
    res.json({ hotelName: adminProfile?.hotelName || 'Aetheria Heights' });
  } catch (err) {
    console.error(err);
...
app.delete('/api/offers/:id', async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(400).json({ error: 'Failed to delete offer', details: err.message });
  }
});
