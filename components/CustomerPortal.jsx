...
const CustomerPortal = ({ rooms, halls, menuItems, bookings, diningOrders, offers = [], user, profile, adminProfile, onCreateBooking, onAddDiningOrder, onAddFeedback, onUpdateProfile, onLogout }) => {
    ...
    const handleBookHotel = (hotel) => {
        // Adapt external map hotel object to match internal room schema for the Booking Modal
        const hotelAsRoom = {
            id: hotel._id,
            name: hotel.name,
            type: hotel.name, // Needed for receipt PDF Generation
            headline: hotel.subtitle,
            description: `Book your luxurious stay at ${hotel.name}. Enjoy premium amenities and world-class service in ${hotel.subtitle.split('in ').pop() || 'this location'}.`,
            price: hotel.pricePerNight,
            capacity: 2,
            size: "Standard Suite",
            amenities: ["Wi-Fi", "AC", "Room Service", "TV"],
            images: [hotel.image, hotel.image, hotel.image]
        };

        setSelectedRoom(hotelAsRoom);
        setSelectedHall(null);
        setCurrentImageIndex(0);
        setStep(1);
        setPaymentMethod('CARD');
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
                <div>
                    ...
                    <p className="text-gray-300 leading-relaxed mb-6">
                        {adminProfile?.hotelName || 'Aetheria Heights'} was born from a singular vision: to create a sanctuary where the friction of travel dissolves, leaving only the experience. We believe that true luxury is seamlessness.
                    </p>
        ...
        <div className="max-w-4xl mx-auto py-24 px-4 min-h-[80vh] text-gray-300 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white/5 border border-white/10 p-8 rounded-xl space-y-6">
                ...
                <section>
                    ...
                    <p>The AI Concierge is provided as a convenience tool. While we strive for accuracy, {adminProfile?.hotelName || 'Aetheria Heights'} is not liable for misunderstandings arising from AI interactions.</p>
                </section>
            </div>
        </div>
    );
        ...
        <div className="max-w-4xl mx-auto py-24 px-4 min-h-[80vh]">
            ...
            <div className="space-y-6">
                {[
                    ...
                    { q: "Are pets allowed?", a: `${adminProfile?.hotelName || 'Aetheria Heights'} is a pet-friendly establishment. A small cleaning fee applies for furry companions.` },
    ...
    return (
        <div className="min-h-screen bg-aetheria-navy text-white font-sans selection:bg-aetheria-gold selection:text-aetheria-navy flex flex-col">
            ...
            <div className="pt-20 flex-grow">
                    ...
                    <>
                        <div className="relative h-[60vh] w-full overflow-hidden flex items-center justify-center">
                            ...
                            <div className="relative text-center px-4 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                                ...
                                <h1 className="text-6xl md:text-7xl font-serif font-bold text-white mb-6 drop-shadow-2xl">{adminProfile?.hotelName || 'Aetheria Heights'}</h1>
                        ...
                        <div className="sticky top-[72px] z-40 bg-[#0b1528]/85 backdrop-blur-xl border-b border-white/10 py-5 px-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-in fade-in duration-500">
                            <div className="max-w-7xl mx-auto flex flex-col xl:flex-row gap-5 items-center justify-between bg-white/[0.02] border border-white/10 p-3 rounded-2xl shadow-inner shadow-black/40">
                                <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto flex-grow">
                                    <div className="flex-1 flex items-center gap-3.5 bg-white/[0.03] hover:bg-white/[0.06] px-4 py-3 rounded-xl border border-white/5 hover:border-aetheria-gold/30 transition-all duration-300 min-w-[240px] shadow-sm">
                                        ...
                                        <div className="flex flex-col w-full">
                                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider flex items-center gap-1">Check In <Clock className="w-3 h-3 text-aetheria-gold/70" /></span>
                                    ...
                                    <div className="flex-1 flex items-center gap-3.5 bg-white/[0.03] hover:bg-white/[0.06] px-4 py-3 rounded-xl border border-white/5 hover:border-aetheria-gold/30 transition-all duration-300 min-w-[240px] shadow-sm">
                                        ...
                                        <div className="flex flex-col w-full">
                                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider flex items-center gap-1">Check Out <Clock className="w-3 h-3 text-aetheria-gold/70" /></span>
                                    ...
                                    <div className="flex-1 flex items-center gap-3.5 bg-white/[0.03] hover:bg-white/[0.06] px-4 py-3 rounded-xl border border-white/5 hover:border-aetheria-gold/30 transition-all duration-300 min-w-[160px] shadow-sm">
                                        <div className="p-2.5 bg-aetheria-gold/10 rounded-lg text-aetheria-gold flex-shrink-0">
                                ...
                                <button onClick={() => document.getElementById('rooms-section')?.scrollIntoView({ behavior: 'smooth' })} className="w-full xl:w-auto bg-gradient-to-r from-aetheria-gold via-[#e5c158] to-aetheria-gold hover:from-white hover:to-white text-aetheria-navy hover:text-aetheria-navy font-bold uppercase tracking-wider py-4 px-9 rounded-xl transition-all duration-300 shadow-[0_0_20px_rgba(212,175,55,0.2)] hover:shadow-[0_0_30px_rgba(212,175,55,0.4)] hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2.5 whitespace-nowrap">
                                ...
                                {filteredRooms.map((room) => (
                                    <div key={room.id} onClick={() => handleBookRoomClick(room)} className="group cursor-pointer bg-white/5 rounded-xl overflow-hidden border border-white/10 hover:border-aetheria-gold/50 transition-all duration-300 hover:transform hover:-translate-y-2">
                                        <div className="h-64 overflow-hidden relative">
                                            <ImageCarousel images={room.images} altText={room.type} imgClassName="transition-transform duration-700 group-hover:scale-110" />
                                            ...
                                            {room.panoramicImages && room.panoramicImages.length > 0 && (
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); setPanoramaImage(room.panoramicImages); }}
                                                    className="absolute bottom-4 right-4 bg-aetheria-gold text-aetheria-navy hover:bg-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors shadow-lg z-10"
                                                >
                                                    <Globe className="w-4 h-4" /> 360° View
                                                </button>
                                            )}
                                        </div>
                                        <div className="p-6">
                        ...
                        {offers.length > 0 && (
                            <section className="max-w-7xl mx-auto py-12 px-4">
                                <div className="flex items-center gap-3 mb-8">
                                    <Sparkles className="w-8 h-8 text-aetheria-gold" />
                                    <h2 className="text-3xl font-serif font-bold text-white">Exclusive Offers</h2>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {offers.filter(o => o.isActive).map((offer, idx) => (
                                        <div key={idx} className="relative rounded-2xl overflow-hidden group border border-white/10 hover:border-aetheria-gold/50 transition-all duration-300">
                                            <div className="absolute inset-0 bg-black/60 group-hover:bg-black/40 transition-colors z-10" />
                                            <img src={offer.imageUrl || 'https://picsum.photos/1000/600'} alt={offer.title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                                            <div className="relative z-20 p-8 h-full flex flex-col justify-end min-h-[300px]">
                                                <div className="bg-aetheria-gold text-aetheria-navy font-bold px-3 py-1 rounded inline-block w-max mb-4 uppercase tracking-wider text-xs">
                                                    {offer.discountPercentage}% OFF
                                                </div>
                                                <h3 className="text-2xl font-serif font-bold text-white mb-2">{offer.title}</h3>
                                                <p className="text-gray-200 mb-6 max-w-md">{offer.description}</p>
                                                <p className="text-xs text-aetheria-gold flex items-center gap-2 font-bold tracking-wider">
                                                    <Clock className="w-4 h-4" /> VALID UNTIL: {new Date(offer.validUntil).toLocaleDateString()}
                                                </p>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <section className="py-24 px-4 bg-gradient-to-b from-aetheria-navy to-black relative overflow-hidden">
                            ...
                            <div className="max-w-4xl mx-auto relative z-10 text-center">
                                ...
                                <p className="text-xs text-gray-600 mt-6">
                                    By subscribing, you agree to receive communications from {adminProfile?.hotelName || 'Aetheria Heights'}.
                                ...
                                {halls.map((hall, idx) => (
                                    <div key={hall.id} className={`flex flex-col md:flex-row gap-8 items-center bg-white/5 border border-white/10 rounded-2xl overflow-hidden p-6 hover:border-aetheria-gold/30 transition-colors ${idx % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
                                        <div className="w-full md:w-1/2 h-80 rounded-xl overflow-hidden relative group">
                                            <ImageCarousel images={hall.images} altText={hall.name} imgClassName="transition-transform duration-700 group-hover:scale-105" />
                                            ...
                                            {hall.panoramicImages && hall.panoramicImages.length > 0 && (
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); setPanoramaImage(hall.panoramicImages); }}
                                                    className="absolute bottom-4 right-4 bg-aetheria-gold text-aetheria-navy hover:bg-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors shadow-lg z-10"
                                                >
                                                    <Globe className="w-4 h-4" /> 360° View
                                                </button>
                                ...
                                {menuItems.filter(item => item.category === diningCategory).map(item => (
                                    <div key={item.id} className="flex gap-4 bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-aetheria-gold/30 transition-all group p-4">
                                        <div className="w-32 h-32 flex-shrink-0 rounded-lg overflow-hidden relative">
                                            <ImageCarousel images={item.images} altText={item.name} imgClassName="group-hover:scale-110 transition-transform duration-500" />
                ...
                {view === 'HOTELS_MAP' && (
                    <div className="max-w-7xl mx-auto py-8 px-4 h-full flex flex-col">
                        <div className="mb-6">
                            <h1 className="text-4xl font-serif font-bold text-white mb-2">Discover Our Locations</h1>
                            <p className="text-gray-400">Find {adminProfile?.hotelName || 'Aetheria Heights'} properties and explore the world of luxury.</p>
                        </div>
                        <HotelMap onBookHotel={handleBookHotel} />
                    </div>
                )}
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="bg-white/5 p-4 rounded-lg flex flex-col md:flex-row gap-4">
                                            <div className="w-32 h-32 rounded overflow-hidden flex-shrink-0">
                                                <ImageCarousel
                                                    images={selectedRoom ? selectedRoom.images : selectedHall.images}
                                                    altText="Preview"
                                                />
                                            </div>
                                            <div>
            ...
            {/* 360 Panorama Viewer */}
            {
                panoramaImage && (
                    <PanoramaViewer
                        imageUrl={Array.isArray(panoramaImage) ? undefined : panoramaImage}
                        images={Array.isArray(panoramaImage) ? panoramaImage : undefined}
