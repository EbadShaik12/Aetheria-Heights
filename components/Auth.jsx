...
const Auth = ({ onLogin }) => {
    ...
    const [dynamicHotelName, setDynamicHotelName] = useState('Aetheria Heights');

    useEffect(() => {
        fetch(`${API_BASE}/api/hotel-config`)
            .then(res => res.json())
            .then(data => {
                if (data.hotelName) {
                    setDynamicHotelName(data.hotelName);
                }
            })
            .catch(console.error);
    }, []);
            ...
            <div className="relative z-10 w-full max-w-md px-6 animate-in fade-in zoom-in duration-500">
                ...
                <div className="flex mb-6 bg-black/40 backdrop-blur-md p-1 rounded-full border border-white/10 mx-auto max-w-[300px]">
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-full text-xs font-bold transition-all duration-300 ${userType === 'GUEST'
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-full text-xs font-bold transition-all duration-300 ${userType === 'ADMIN'
                    ...
                    <div className="text-center mb-8">
                        ...
                        <h2 className="text-3xl font-serif text-white font-bold mb-2">
                            {showForgotPassword
                                ...
                                : (userType === 'ADMIN' ? 'Staff Portal' : dynamicHotelName)
                            }
                        </h2>
