...
const ImageCarousel = ({ images, altText, className = "", imgClassName = "" }) => {
    ...
    if (!images || images.length === 0) {
        return <div className={`bg-slate-800 w-full h-full ${className}`}></div>;
    }

    return (
        <div className={`relative group/carousel w-full h-full overflow-hidden ${className}`}>
            <img src={images[currentIndex]} alt={altText} className={`w-full h-full object-cover ${imgClassName}`} />
            ...
            {images.length > 1 && (
                <>
                    <button
                        type="button"
                        onClick={prevImage}
                        className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 text-white p-1.5 rounded-full opacity-0 group-hover/carousel:opacity-100 transition-opacity hover:bg-black/80 z-20 pointer-events-auto"
                    >
                    ...
                    <button
                        type="button"
                        onClick={nextImage}
                        className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 text-white p-1.5 rounded-full opacity-0 group-hover/carousel:opacity-100 transition-opacity hover:bg-black/80 z-20 pointer-events-auto"
                    >
...
const AdminPortal = ({
    rooms, halls, menuItems, bookings, profiles, feedbacks = [], offers = [],
    ...
    const [newRoom, setNewRoom] = useState({
        number: '', type: 'Deluxe Suite', category: 'General', price: 0, maxCapacity: 2, description: '', amenities: [], status: 'Clean', images: [], panoramicImages: []
    });
    ...
    const [newOffer, setNewOffer] = useState({
        title: '', description: '', discountPercentage: 10, validUntil: '', isActive: true, imageUrl: ''
    });
    ...
    const handleOpenAddRoom = () => {
        ...
        setIsEditingRoom(false);
        setNewRoom({ number: '', type: 'Deluxe Suite', category: 'General', price: 0, maxCapacity: 2, description: '', amenities: [], status: 'Clean', images: [], panoramicImages: [] });
        setAmenitiesInput('');
    ...
    const submitRoom = (e) => {
        ...
        const roomData = {
            ...
            panoramicImages: newRoom.panoramicImages || []
        };
    ...
    const handleOpenAddHall = () => {
        ...
        setNewHall({ name: '', type: 'Wedding', pricePerHour: 0, capacity: 50, status: 'Available', description: '', features: [], images: [], panoramicImages: [] });
        setFeaturesInput('');
    ...
    const submitHall = (e) => {
        ...
        const hallData = {
            ...
            panoramicImages: newHall.panoramicImages || []
        };
    ...
    const handleOpenAddOffer = () => {
        ...
        setNewOffer({ title: '', description: '', discountPercentage: 10, validUntil: '', isActive: true, imageUrl: '' });
    ...
    const handleOpenEditOffer = (offer) => {
        ...
        setNewOffer({ ...offer, validUntil: formattedDate });
        setIsOfferModalOpen(true);
    };

    const submitOffer = (e) => {
        e.preventDefault();
        if (!newOffer.title || !newOffer.discountPercentage || !newOffer.validUntil) return;

        const offerData = {
            id: isEditingOffer ? currentOfferId : `OFFER-${Date.now()}`,
            ...
            imageUrl: newOffer.imageUrl || 'https://picsum.photos/800/600?random=off'
        };
                ...
                Array.from(files).forEach(file => {
                    ...
                    reader.onloadend = () => {
                        if (reader.result) {
                            ...
                            } else if (type === 'ROOM_PANO') {
                                setNewRoom(prev => ({ ...prev, panoramicImages: [...(prev.panoramicImages || []), reader.result] }));
                            } else if (type === 'HALL') {
                                setNewHall(prev => ({ ...prev, images: [...(prev.images || []), reader.result] }));
                            } else if (type === 'HALL_PANO') {
                                setNewHall(prev => ({ ...prev, panoramicImages: [...(prev.panoramicImages || []), reader.result] }));
                            } else if (type === 'MENU') {
                                setNewMenuItem(prev => ({ ...prev, images: [...(prev.images || []), reader.result] }));
                            } else if (type === 'OFFER') {
                                setNewOffer(prev => ({ ...prev, imageUrl: reader.result }));
                            }
                        }
                    };
                    reader.readAsDataURL(file);
    ...
    const removeImage = (index, type) => {
        if (type === 'ROOM') {
            ...
            setNewRoom(prev => ({ ...prev, panoramicImages: prev.panoramicImages?.filter((_, i) => i !== index) }));
        } else if (type === 'HALL') {
            setNewHall(prev => ({ ...prev, images: prev.images?.filter((_, i) => i !== index) }));
        } else if (type === 'HALL_PANO') {
            setNewHall(prev => ({ ...prev, panoramicImages: prev.panoramicImages?.filter((_, i) => i !== index) }));
        } else if (type === 'OFFER') {
    ...
    const renderOffers = () => (
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-800/30">
                <h2 className="text-xl font-bold text-white flex items-center gap-2"><Gift className="w-5 h-5 text-aetheria-gold" /> Offers & Packages</h2>
                <button onClick={handleOpenAddOffer} className="bg-aetheria-gold hover:bg-aetheria-gold/90 text-aetheria-navy px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-colors">
                    <Plus className="w-4 h-4" /> Add Offer
                </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                {offers.map(offer => (
                    <div key={offer.id} className="bg-[#0B1120] border border-slate-800 rounded-lg overflow-hidden flex flex-col group">
                        <div className="h-48 relative">
                            <img src={offer.imageUrl || 'https://picsum.photos/800/600?grayscale'} alt={offer.title} className="w-full h-full object-cover" />
                            <div className="absolute top-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded select-none shadow">
                                {offer.discountPercentage}% OFF
                            </div>
                            {!offer.isActive && (
                                <div className="absolute inset-0 bg-red-900/40 flex items-center justify-center">
                                    <span className="text-red-200 font-bold bg-red-900/80 px-3 py-1 rounded uppercase tracking-wider text-xs border border-red-700">INACTIVE</span>
                                </div>
                            )}
                        </div>
                        <div className="p-5 flex-1 flex flex-col">
                            <h3 className="text-lg font-bold text-white mb-2">{offer.title}</h3>
                            <p className="text-sm text-slate-400 mb-4 line-clamp-3 flex-1">{offer.description}</p>
                            <div className="flex items-center justify-between text-xs text-slate-500 mb-4 pt-4 border-t border-slate-800">
                                <span>Valid until: {new Date(offer.validUntil).toLocaleString()}</span>
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => handleOpenEditOffer(offer)} className="flex-1 border border-slate-700 hover:bg-slate-800 text-slate-300 py-2 rounded flex items-center justify-center gap-2 transition-colors"><Pencil className="w-4 h-4" /> Edit</button>
                                <button onClick={() => { if (window.confirm('Are you sure you want to delete this offer?')) onDeleteOffer(offer.id); }} className="px-3 border border-red-900/50 hover:bg-red-900/20 text-red-500 rounded flex items-center justify-center transition-colors"><Trash2 className="w-4 h-4" /></button>
                            </div>
                        </div>
                    </div>
                ))}
                {offers.length === 0 && (
                    <div className="col-span-full py-12 text-center text-slate-500">
                        ...
                        <p>No active offers found. Create one to attract more bookings.</p>
                    </div>
                )}
            </div>
        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-[#0B1120] border border-aetheria-gold/30 rounded-xl shadow-2xl w-full max-w-lg p-8 overflow-y-auto max-h-[90vh] glass-scroll">
                        ...
                        <form onSubmit={submitRoom} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-xs uppercase text-slate-500 mb-1">360° Panoramic Images</label>
                                <div className="flex flex-col gap-2">
                                    <label className="flex items-center gap-2 cursor-pointer bg-slate-800 hover:bg-slate-700 transition-colors border border-slate-700 rounded-lg p-3 w-full border-dashed">
                                        <Camera className="w-5 h-5 text-aetheria-gold" />
                                        <span className="text-sm text-slate-300">Upload or Capture 360° Photo</span>
                                        <input
                                            type="file"
                                            multiple
                                            accept="image/*"
                                            capture="environment"
                                            onChange={(e) => handleImageUpload(e, 'ROOM_PANO')}
                                            className="hidden"
                                        />
                                    </label>
                                    <div className="flex gap-2 overflow-x-auto py-2">
                                        {(newRoom.panoramicImages || []).map((img, i) => (
                                            <div key={i} className="relative w-16 h-16 flex-shrink-0 group ring-1 ring-aetheria-gold rounded overflow-hidden">
                                                <img src={img} alt="360" className="w-full h-full object-cover" />
                                                <div className="absolute inset-0 flex items-center justify-center bg-black/40"><div className="text-[8px] text-white font-bold bg-black/50 px-1 rounded">360°</div></div>
                                                <button type="button" onClick={() => removeImage(i, 'ROOM_PANO')} className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white"><X className="w-4 h-4" /></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <form onSubmit={submitHall} className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="flex items-center gap-2 cursor-pointer bg-slate-800 hover:bg-slate-700 transition-colors border border-slate-700 rounded-lg p-3 w-full border-dashed">
                                        <Camera className="w-5 h-5 text-aetheria-gold" />
                                        <span className="text-sm text-slate-300">Upload or Capture 360° Hall Photo</span>
                                        <input
                                            type="file"
                                            multiple
                                            accept="image/*"
                                            capture="environment"
                                            onChange={(e) => handleImageUpload(e, 'HALL_PANO')}
                                            className="hidden"
                                        />
                                    </label>
                                    <div className="flex gap-2 overflow-x-auto py-2">
                                        {(newHall.panoramicImages || []).map((img, i) => (
                                            <div key={i} className="relative w-16 h-16 flex-shrink-0 group ring-1 ring-aetheria-gold rounded overflow-hidden">
                                                <img src={img} alt="360" className="w-full h-full object-cover" />
                                                <div className="absolute inset-0 flex items-center justify-center bg-black/40"><div className="text-[8px] text-white font-bold bg-black/50 px-1 rounded">360°</div></div>
                                                <button type="button" onClick={() => removeImage(i, 'HALL_PANO')} className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white"><X className="w-4 h-4" /></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
            ...
            {/* Add/Edit Offer Modal */}
            {
                isOfferModalOpen && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setIsOfferModalOpen(false)}></div>
                        <div className="relative bg-[#0B1120] border border-aetheria-gold/30 rounded-xl shadow-2xl w-full max-w-lg p-8 overflow-y-auto max-h-[90vh] glass-scroll">
                            <h3 className="text-xl font-bold text-white mb-6">{isEditingOffer ? 'Edit Offer' : 'Add New Offer'}</h3>
                            <form onSubmit={submitOffer} className="space-y-4">
                                <div><label className="block text-xs uppercase text-slate-500 mb-1">Offer Title</label><input type="text" value={newOffer.title} onChange={e => setNewOffer({ ...newOffer, title: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded p-3 text-white" required /></div>

                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs uppercase text-slate-500 mb-1">Discount %</label>
                                        <input type="number" min="0" max="100" value={newOffer.discountPercentage} onChange={e => setNewOffer({ ...newOffer, discountPercentage: Number(e.target.value) })} className="w-full bg-slate-900 border border-slate-700 rounded p-3 text-white" required />
                                    </div>
                                    <div>
                                        <label className="block text-xs uppercase text-slate-500 mb-1">Valid Until</label>
                                        <input type="datetime-local" value={newOffer.validUntil} onChange={e => setNewOffer({ ...newOffer, validUntil: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded p-3 text-white" required />
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 mt-2">
                                    <input type="checkbox" id="isActiveOffer" checked={newOffer.isActive} onChange={e => setNewOffer({ ...newOffer, isActive: e.target.checked })} className="w-4 h-4 accent-aetheria-gold" />
                                    <label htmlFor="isActiveOffer" className="text-slate-300 select-none">Offer is Active</label>
                                </div>
                                <div><label className="block text-xs uppercase text-slate-500 mb-1">Description</label><textarea rows={3} value={newOffer.description} onChange={e => setNewOffer({ ...newOffer, description: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded p-3 text-white" required /></div>
                                <div>
                                    <label className="block text-xs uppercase text-slate-500 mb-1">Banner Image (Optional)</label>
                                    <div className="flex flex-col gap-2">
                                        <input
                                            type="file"
                                            accept="image/*"
                                        ...
                                        {newOffer.imageUrl && (
                                            <div className="relative w-32 h-20 group">
                                                <img src={newOffer.imageUrl} alt="" className="w-full h-full object-cover rounded shadow" />
                                                <button type="button" onClick={() => removeImage(0, 'OFFER')} className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white rounded"><X className="w-5 h-5" /></button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <div className="pt-4 flex justify-end gap-3">
                                    <button type="button" onClick={() => setIsOfferModalOpen(false)} className="px-6 py-2 border border-slate-700 text-slate-300 rounded hover:bg-slate-800 transition-colors">Cancel</button>
                                    <button type="submit" className="px-6 py-2 bg-aetheria-gold text-aetheria-navy font-bold rounded hover:bg-yellow-500 transition-colors">{isEditingOffer ? 'Save Changes' : 'Create Offer'}</button>
                                </div>
                            </form>
                        </div>
                    </div>
